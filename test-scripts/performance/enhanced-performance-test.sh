...
# Enhanced Comprehensive API Testing Script for Talksy Application
# Tests all endpoints, measures latencies, and generates detailed reports
...
mkdir -p $REPORTS_DIR
REPORT_FILE="$REPORTS_DIR/enhanced_api_test_report_$(date +%Y%m%d_%H%M%S).txt"
LOG_FILE="$REPORTS_DIR/enhanced_api_test_log_$(date +%Y%m%d_%H%M%S).log"
...
# Initialize report
echo "Talksy API Enhanced Test Report - $(date)" > $REPORT_FILE
echo "========================================" >> $REPORT_FILE
...
test_websocket_info() {
    ...
    print_header "WebSocket Information"
    print_status "INFO" "WebSocket endpoint available at: ws://localhost:3000"
    print_status "INFO" "WebSocket functionality requires a dedicated WebSocket client for testing"
    print_status "INFO" "The application supports real-time AI assistant features via WebSocket"
    
    echo "WebSocket Information:" >> $REPORT_FILE
    echo "  Endpoint: ws://localhost:3000" >> $REPORT_FILE
    echo "  Features: Real-time AI assistant, message streaming, tool execution" >> $REPORT_FILE
}

# Main test execution
main() {
    print_header "Talksy API Comprehensive Testing Suite"
